---
description: Execute the Power Trio plan phase only. Use proactively when creating a plan for a new feature.
...
Execute Phase 1: Plan with the Power Trio committee.
...
5. **Loop Termination Bias**:
   ...
   - Remember: Phase 1 is expected to converge quickly (see `MAX_PLAN_ITERATIONS`)
   - Long, exhaustive analyses are a smell - **concision is a quality signal**

6. **Loop back to Step 4** (Vote Collection)

**Maximum iterations: `MAX_PLAN_ITERATIONS`**. If no consensus after max iterations, output current state with unresolved concerns and ask user for guidance.
